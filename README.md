...
### 🚀 Новая версия 3.0 доступна для тестирования на базе докер !
-----
Чтобы приступить к работе, выполните несколько простых шагов:

1. Скачайте файл deploy.sh с помощью команды curl:
   $ curl -sLo deploy.sh https://raw.githubusercontent.com/kootik/f152z/refactor/docker-ci/install/deploy.sh

2. Придайте файлу права на выполнение с помощью команды chmod:
   $ chmod +x deploy.sh

3. Запустите установку с помощью команды./deploy.sh:
   $./deploy.sh --install-docker --start

Для удобства, вы можете автоматизировать процесс:
...
>     ```
>
> 2.  **Переключение на ветку `v2.0`:**
>     Это самая важная команда. Она переключит вас на новую версию кода.
>
>     ```bash
>     git checkout v2.0
>     ```
...
>     Следуйте стандартным инструкциям по установке, описанным ниже в разделе **"Установка и запуск"**.
