...
## Changelog: Новая версия vs. GitHub-версия

Это крупное обновление, которое полностью перерабатывает как внутреннюю (backend), так и внешнюю (frontend) архитектуру приложения, превращая его из простого инструмента для сбора результатов в полноценную платформу для прокторинга, аналитики и управления данными.

### Backend (`app.py`)

Архитектура бэкенда была значительно улучшена, сделав приложение более производительным, безопасным и масштабируемым.

  * **База данных и Схема:**

      * **Переход на нормализованную БД:** Вместо простого хранения JSON-файлов, система теперь использует реляционную базу данных SQLite с чётко определённой схемой.
      * **Новые таблицы:** Добавлены таблицы для централизованного хранения пользователей (`users`), цифровых отпечатков (`fingerprints`), метаданных результатов (`result_metadata`), событий прокторинга (`proctoring_events`) и выданных сертификатов (`certificates`).
      * **Оптимизация производительности:** Запросы на получение списка результатов теперь выполняются через быструю SQL-выборку из таблицы `result_metadata`, а не путём чтения и парсинга множества JSON-файлов, что кардинально ускоряет работу аналитической панели.
      * **Надёжность транзакций:** Сохранение результатов теперь происходит в рамках одной атомарной транзакции, обновляя все связанные таблицы (пользователи, отпечатки, результаты, сертификаты), что гарантирует целостность данных.
      * **Управление схемой:** Добавлена система версионирования схемы БД (`PRAGMA user_version`) для упрощения будущих миграций.

  * **API и Безопасность:**

      * **Новые API-эндпоинты:** Добавлены маршруты для получения полного JSON-файла по ID сессии (`/api/get_full_result/<session_id>`) и для серверного анализа движений мыши (`/api/analyze_mouse_from_files`).
      * **Безопасность и Стабильность:** Внедрены современные практики веб-разработки:
          * **CSRF-защита** (`Flask-WTF`) для всех POST-запросов.
          * **Кеширование** (`Flask-Caching`) для ускорения часто запрашиваемых данных (списки результатов, реестр сертификатов).
          * **Ограничение частоты запросов** (`Flask-Limiter`) для защиты от брутфорс-атак и злоупотреблений.
          * **Поддержка WebSockets** (`Flask-Sock`) для уведомлений в реальном времени (например, о новых результатах).
          * **Улучшенная валидация:** Добавлены функции для проверки форматов `session_id` и `event_type` на стороне сервера.
          * **Управление `SECRET_KEY`:** Ключ теперь загружается из переменных окружения, что является стандартом безопасности.

  * **Новая функциональность:**

      * **Серверный анализ движений мыши:** Реализован сложный алгоритм сравнения траекторий мыши с использованием `fastDTW` (Dynamic Time Warping) на стороне сервера. Это позволяет выявлять с высокой точностью случаи списывания, когда разные пользователи показывают аномально схожие паттерны движений.
      * **Улучшенное логирование:** Внедрён стандартный модуль `logging` для более информативного вывода сообщений об ошибках и событиях.

### Frontend (HTML/JS)

Интерфейс был полностью переписан с нуля. Вместо монолитного подхода с большим количеством кода в одном HTML-файле, теперь используется современная модульная архитектура (SPA - Single Page Application).

  * **Архитектура:**

      * **Модульная структура:** Frontend разделён на логические модули: `main.js` (оркестратор), `ui.js` (управление DOM), `api.js` (взаимодействие с сервером) и `analysis.js` (клиентская логика анализа). Это упрощает поддержку и расширение кода.
      * **Аналитическая панель (`display_results.html`):** Старая страница с простой таблицей превратилась в полноценный дашборд с несколькими вкладками:
          * **Дашборд:** Виджеты с ключевыми метриками и графики.
          * **Детальное сравнение:** Мощный инструмент для попарного сравнения сессий, включая визуализацию траекторий мыши и анализ отпечатков.
          * **Прерванные сессии:** Уникальный отчёт, показывающий сессии, которые были начаты, но не завершены.
          * **Поведенческий анализ:** Автоматический отчёт о подозрительных сессиях.
          * **Реестр аттестатов:** Таблица со всеми выданными документами.
          * **Сводный отчёт:** Общая статистика по всем тестам.
      * **Асинхронная загрузка:** Все данные теперь подгружаются асинхронно, что делает интерфейс отзывчивым. Индикаторы загрузки показывают пользователю, что данные в процессе получения.

  * **Пользовательский интерфейс и UX:**

      * **Современный дизайн:** Интерфейс переработан, стал более чистым, интуитивно понятным и информативным.
      * **Интерактивность:** Добавлены сортировка таблиц по клику на заголовок, продвинутые фильтры, модальные окна для настроек и просмотра логов, а также всплывающие уведомления.
      * **Профиль пользователя:** При клике на имя пользователя открывается модальное окно с историей всех его попыток тестирования.

  * **Прокторинг и Сбор данных на клиенте (`117-test.html` и др.):**

      * **Улучшенная логика:** Скрипты на страницах тестов были значительно доработаны для сбора более детальных поведенческих метрик (например, количество смен ответа на вопрос).
      * **Клиентский анализ:** Добавлен файл `analysis.js`, который может выполнять анализ отпечатков и сравнение движений мыши прямо в браузере.

  * **Стартовые страницы (`index.html`, `index-start.html`):**

      * **Улучшенная навигация:** Процесс начала работы стал более структурированным. `index.html` теперь служит для ввода данных о пользователе, которые сохраняются в `localStorage` и предзаполняются при следующем визите, а `index-start.html` — как центральный хаб для выбора учебного модуля.
