...
def test_save_results_with_valid_api_key(client, api_headers):
    response = client.post(
        "/api/save_results", data=json.dumps({"test": "data"}), headers=api_headers
    )
...
def test_log_event(client, db_session):
    ...
    response = client.post(
        "/api/log_event", json=event_data, headers={"X-API-Key": "client-api-key"}
    )
    assert response.status_code == 200
...
def test_get_events_for_session(client, db_session, admin_api_headers):
    ...
    response = client.get(
        f"/api/get_events/{result.session_id}", headers=admin_api_headers
    )
    assert response.status_code == 200
...
def test_get_full_result_not_found(client, admin_api_headers):
    ...
    response = client.get(
        "/api/get_full_result/non-existent-id", headers=admin_api_headers
    )
