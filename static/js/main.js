...
function initializeEventListeners() {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (pageButton && !pageButton.disabled) {
            api.loadInitialData(parseInt(pageButton.dataset.page, 10));
            return;
        }
        ...
        if (logLink) {
            e.preventDefault();
            api.showEventLog(logLink.dataset.sessionId);
            return;
        }
...
async function runDetailedAnalysis() {
    ...
    try {
        ...
        const fetchPromises = selectedIds.map(id => api.fetchFullResultDetails(id));
        ...
        if (fullResults.length > 1) {
            ...
            if (useServer) {
                dtwResults = await api.runServerDtwAnalysis(selectedIds);
            } else {
